...
\fi
This section will give an overview about the SGM and related work that was done to optimize it. It will also give an understanding over the used optimizers in our experiments. 
\section{The Skip-Gram Model} 
The Skip Gram Model is a model used to embed words into vectors, by analyzing the context in which the words happens. It's trained to predict the words that are next to the input word $w$. 
It's achieving this by maximizing the following equation:
...
This equation can be transformed into sums by using log probabilities: 
...
\end{equation}
As at time step $t$ $RMS[\Delta \theta]_{t} $ is unknown, we approximat it with $ RMS[\Delta \theta]_{t}-1$. Know we replac $\eta$ with  $ RMS[\Delta \theta]_{t}-1$ and get the final update rule: 
\begin{equation}

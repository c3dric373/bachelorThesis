...
The Skip Gram Model is a model used to embed words into vectors, by analyzing the context in which the words happens.
% the context in which the words happens_ ich krieg kein Frageyeichen hin. Appear_
...
This equation can be transformed quite easily 
% bewertende Sat<teile wie quite easily w[rde ich glaub ich einfach weglassen. 
into sums by using log probabilities: 
\begin{equation} \sum _{t=1}^T \sum_{-m<j<m} log( p(w_{t+j}|w_t) )\end{equation} 
    where the parameters are the same as in \ref{basicSkip}
% Vor einer Referenz immer die Art der Sache auf die du referenyierst mit angeben, in dem Fall das Wort equation vor backslash ref. Sonst ist das uneindeutig, es gibt sowohl ein Kapitel 2.1, als auch eine Figure 2.1, als auch eine equation 2.1.    
...
\end{figure}
%TODO EXPLAIN SGD GD VANILLA VERY GOOD UNDERSTAND IT REALLY AND ADADELTA AND ADAM ARE TWO IMPROVE

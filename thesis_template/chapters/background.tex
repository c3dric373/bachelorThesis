...
This section will give a quick overview about a few optimizers existing in machine learning. Those optimizers were used to optimize the Skip-Gram Model.
\subsection{SGD}
\subsection{Momentum}
The main Idea behind momentum is to use the previous gradient to compute the current update vector.  We do this, because SGD can have trouble, when the loss function is very steep in only one direction. SGD would then osciliate and only taking small steps into the steep direction.  While using the previous gradient, momentum will accelerate the learning in the correct direction. At the update $t$ we will comppute our update vector $v_t$ the following way:
\begin{equation}
v_t = \gamma v_{t-1} + \alpha \nabla J (\theta)
\end{equation}
And then we update our weights as usual: $\theta = \theta - v_t$
\subsection{Nesterov}
Momentum can be a powerful tool, and sometimes be it's own enemy. With momentum the learning algorithm often overshoots, and blows by the local optima. Hence it will never converge. This problem was addressed by Nesterov. The idea behind his algorithm is to incorporate the momentum in the computation of our gradients. We will substract the momentum vector from our weights before computing the gradients. Therefore we will kind of compute the gradients from the position where we would be with momentum. which will allow us to make a step in a better direction. The computation of the update vector will look the following way:
\begin{equation}
v_t = \gamma v_{t-1} + \alpha \nabla J (\theta -  \gamma v_{t-1})
\end{equation}
.  this have shown to work very well, especially for RNN's. 
\subsection{Adagrad}
